...
describe('Teste a função fetchCharacter', () => {
  ...
  it('Verifica se retorna erro ao executar a função sem parâmetro', async () => {
    const faillRequest = await fetchCharacter();
    expect(faillRequest).toEqual(new Error('You must provide an url'));
  });
  it("Verifica se retorna 'Invalid id' ao executar a função com parâmetro que não existe", async () => {
    const idInvalid = await fetchCharacter('parametro qualquer');
    expect(idInvalid).toBe('Invalid id')
  });
